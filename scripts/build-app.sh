...
/usr/libexec/PlistBuddy -c 'Set :CFBundleName txtr' "$INNER_PLIST"
/usr/libexec/PlistBuddy -c 'Set :CFBundleIdentifier local.txtr' "$INNER_PLIST"
/usr/libexec/PlistBuddy -c 'Set :CFBundleIconFile AppIcon' "$INNER_PLIST"
/usr/libexec/PlistBuddy -c 'Add :CFBundleDisplayName string txtr' "$INNER_PLIST" 2>/dev/null || /usr/libexec/PlistBuddy -c 'Set :CFBundleDisplayName txtr' "$INNER_PLIST"
...
cat > "$APP/Contents/MacOS/txtr" << 'LAUNCH'
#!/bin/bash
DIR="$(cd "$(dirname "$0")" && pwd)"
export PYTHONEXECUTABLE="$DIR/txtr"
exec "$DIR/../Resources/Python.app/Contents/MacOS/Python" "$DIR/../Resources/texter.py" "$@"
LAUNCH
chmod +x "$APP/Contents/MacOS/txtr" "$APP/Contents/Resources/texter.py"

if command -v codesign >/dev/null; then
  codesign --force --deep -s - "$APP" >/dev/null
fi

DEST="$HOME/Applications/txtr.app"
mkdir -p "$HOME/Applications"
rm -rf "$DEST" "$HOME/Applications/Textpress.app" "$HOME/Applications/Texter.app"
cp -R "$APP" "$DEST"
echo "$APP"
echo "$DEST"
